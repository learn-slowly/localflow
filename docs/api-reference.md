...
- **응답 항목**:
  ...
  |------|------|
  | opratDate | 운행일자 |
  | stgSdCd/stgSdNm | 출발지 시/도 코드/명 |
  | stgSggCd/stgSggNm | 출발지 시/군/구 코드/명 |
  | stgEmdCd/stgEmdNm | 출발지 읍/면/동 코드/명 |
  | arrSdCd/arrSdNm | 도착지 시/도 코드/명 |
  | arrSggCd/arrSggNm | 도착지 시/군/구 코드/명 |
  | arrEmdCd/arrEmdNm | 도착지 읍/면/동 코드/명 |
  | tzon | 시간대 |
  | quater | 15분 단위 |
  | useStf | 이용인원수 |
  | useTm | 평균 통행시간 |
